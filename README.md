...
|---|---|---|---|
| Mock planner | 2/8 (25%) | **6/6 (100%)** | 12.4 s |
| Real Claude | 1/7 (14%) | **8/8 (100%)** | 26.6 s |

```mermaid
xychart-beta
    title "Plan feasibility: pre-rebuild artifact vs post-rebuild (%)"
    x-axis ["mock before", "mock after", "claude before", "claude after"]
    y-axis "feasible (%)" 0 --> 100
    bar [25, 100, 14, 100]
```

### Orchestration evaluation — what the gate actually measures now
...
```mermaid
xychart-beta
    title "Orchestrator metrics (mock vs Claude)"
    x-axis ["End-to-end success", "Infeasible-task recognition", "Transient-failure recovery", "Decomposition validity", "Invalid-plan catch rate"]
    y-axis "score" 0 --> 1
    bar [0.60, 1.00, 1.00, 1.00, 1.00]
    bar [1.00, 1.00, 1.00, 1.00, 1.00]
```
...
xychart-beta
    ...
    x-axis ["1", "4", "16", "64", "256", "1024"]
    y-axis "efficiency" 0.5 --> 1
    line [1.000, 0.993, 0.968, 0.902, 0.892, 0.868]
```
...
_This section is generated by `benchmarks/render_readme.py` from the JSONs in `benchmarks/*/results/` — regenerate it after re-running any benchmark; do not edit it by hand._
